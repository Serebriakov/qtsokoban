...
PlayField::PlayField(QWidget *parent, const char *name, Qt::WFlags f)
  ...
  setFocus();
  setBackgroundMode(/*Qt::NoBackground*/Qt::FixedColor);
  ...
  animDelay_ = settings.value("settings/animDelay", 2).asInt();
...
PlayField::~PlayField() {
  ...
  QSettings settings("Sokoban.ini", QSettings::IniFormat);
  settings.setValue("settings/animDelay", animDelay_);
  settings.sync();
  delete mapDelta_;
  delete history_;
  delete levelMap_;
...
PlayField::changeCursor(const QCursor c) {
  //if ((const QCursor)cursor_ == c) return;
...
PlayField::highlight() {
  ...
  if (pathFinder_.canDrag(x, y)) {
	...
	else changeCursor(Qt::PointingHandCursor);
    if (highlightX_ >= 0) {
      QPainter paint(this);
...
PlayField::startMoving(MoveSequence *ms) {
  ...
  if (animDelay_){
	  movetimerid_ = startTimer(delay[animDelay_]);
  }
  //timerEvent(0);
}

void
PlayField::timerEvent(QTimerEvent *event) {
  ...
  if (Movieplaying){
	if (animDelay_) {
	  if(!moveSequence_->next()) 
		moveSequence_ = history_->deferRedo(levelMap_);
	  if (moveSequence_){ more = true;}
	  else {
	    Movieplaying = false;
		widget->MovieEnd();
	  }
  ...
  if (more) {
    ...
    if (levelMap_->completed()) {
      ...
      stopMoving();
	  // add code£¬to write push step¡£
	  Bookmark completed_bm(collection()->id(), levelMap_->level());
	  completed_bm.completed_set(collection()->id(), levelMap_->level(), levelMap_->totalMoves(), history_);
	  ModalLabel::message(i18n("Level completed"), this/*, &imageData_->idk()*/);
      nextLevel();
	  repaint();
      return;
    }
...
PlayField::stopDrag() {
  ...
  changeCursor(Qt::PointingHandCursor);
...
PlayField::setBookmark(Bookmark *bm) {
  ...
  if (collection()->id() < 0) {
    QMessageBox::warning(this, "sorry", i18n("Sorry, bookmarks for external levels\n"
...
bool
PlayField::playHistory() {

  restartLevel();  
  Bookmark bm(collection()->id(), levelMap_->level());
  if (!bm.completed_goTo(levelMap_, history_)){ return false; }
  MoveSequence *ms = history_->deferRedo(levelMap_);
  if(ms && canMoveNow()) {
    Movieplaying = true;
	startMoving(ms);
	return true;
