...
void 
Bookmark::completed_fileName(QString &p) {
  p = "./Collection-";

  QString n;
  n.setNum(collection_);
  p = p + n;
  QDir collectionDir(NULL);
  if (!collectionDir.exists(p)) collectionDir.mkdir(p);
  p = p + "/complete-" + n;
  n.setNum(level_);
  p = p + "-" + n; 
}
...
Bookmark::Bookmark(int _collection, int _level) :
  number_(-1), collection_(_collection), level_(_level), moves_(0), data_("") {
  QString p;
  completed_fileName(p);
...
void 
Bookmark::completed_set(int _collection, int _level, int _moves, History *_h) {
...
bool
Bookmark::completed_goTo(LevelMap *_map, History *_h) {
  bool canundo;
  if (_h->load(_map, data_.latin1())!=0) {
	while(_h->undo(_map)){;}
	return true;  
  }else{
	return false;
