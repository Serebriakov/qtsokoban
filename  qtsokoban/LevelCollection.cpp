...
LevelCollection::loadPrefs() {
  if (id_ >= 0) {
    ...
    level_ = settings.value(key, 0).asInt();

    key.sprintf("status%d", id_);
    unsigned long x = settings.value(key, 0).asInt();
    ...
    if (!settings.contains (key)) completedLevels_ = 0;
...
LevelCollection::levelCompleted() {
  ...
  if (id_ >= 0) {
    qulonglong  x=(((qulonglong ) 0/*getuid()*/) & 0xfffful);
    x |= ((qulonglong ) id_)<<26;
    x |= ((qulonglong ) completedLevels_)<<16;
